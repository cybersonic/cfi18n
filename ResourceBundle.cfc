<cfcomponent displayname="resourceBundle" hint="reads and parses java resource bundle per locale, version 1.2 icu4j may-2006" output="no">
<!--- 
...
revisions:	15-mar-2005	fixed un-scoped var variable in formatRBString method.
			4-mar-2006	added messageFormat, verifyPattern methods
			9-mar-2006 added public init to pass location of icu4j jar file
...
notes:		the purpose of this CFC is to extract text resources from a pure java resource bundle. these
			resource bundles should be produced by a tools such as IBM's rbManager and consist of:
				...
				Cancel=\u0E22\u0E01\u0E40\u0E25\u0E34\u0E01
				Go=\u0E44\u0E1B
			requires the use of the icu4j java library.				

methods in this CFC:
	-init initializes this CFC. optional argument is jarFile, string holding full-path to icu4j 
	jar file. PUBLIC
...
<cfscript>
	...
	variables.msgFormat=loader.create("com.ibm.icu.text.MessageFormat");
	...
	variables.RBVersion="1.2 icu4j";
	variables.RBDate="may-2006"; //should be date of latest change	
	return this;
</cfscript>
</cffunction>
...
<cffunction name="messageFormat" access="public" output="no" returnType="string" hint="performs messageFormat on compound rb string">
	...
	<cfargument name="thisLocale" required="no" default="en_US" hint="locale to use in formatting, defaults to en_US">
		...
		<cfset var tLocale="">
		<cfset var initArgs=arrayNew(1)>
		<cftry>
			<cfset tLocale=getICU4JLocale(arguments.thisLocale)>
			<cfif NOT isArray(inputArgs)>
				<cfset inputArgs=listToArray(inputArgs)>
			</cfif>	
			<cfset thisFormat=getICU4JMsgFormat(arguments.thisPattern,tLocale)>
...
<cffunction name="verifyPattern" access="public" output="no" returnType="boolean" hint="performs verification on MessageFormat pattern">
	...
	<cfscript>
		...
		var isOK=true;
		try {
			test=variables.msgFormat.newInstance(listToarray(arguments.pattern).toArray());
		}
		catch (Any e) {
			isOK=false;
		}
...
<cfscript>
	...
	version.RBVersion=RBVersion;
	version.RBDate=RBDate;
